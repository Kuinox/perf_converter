...
{
    [Test]
    public void SortsSiblingEndBeforeNextBeginAtSameTimestamp()
    {
        var endpoints = new[]
        {
        ...
        Assert.That(sorted[1].FrameId, Is.EqualTo(2));
        Assert.That(sorted[2].IsBegin, Is.True);
        Assert.That(sorted[2].FrameId, Is.EqualTo(3));
    }

    [Test]
    public void SortsNestedEndsFromDeepestToShallowestAtSameTimestamp()
    {
        var endpoints = new[]
        {
            ...
            new Processor.SliceEndpoint(Time: 20, Trace: 4, Depth: 2, FrameId: 2, IsBegin: false),
        };
